package com.testapplications.proximity;
...
public class MainActivity extends AppCompatActivity implements SensorEventListener, AdapterView.OnItemSelectedListener {
    ...
    public void buttonRecordData(View view) {
        if (!isRecording) {
            ...
            mSensorManager.registerListener(this, mSensorProximity, interval);
        }
        else {
            isRecording = false;
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (printData.isChecked()) {
            ((TextView)findViewById(R.id.proxValue)).setText(String.format("%.5f",event.values[0]));
        }
