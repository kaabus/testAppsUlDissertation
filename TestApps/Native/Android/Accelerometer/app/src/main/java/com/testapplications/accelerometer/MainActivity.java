package com.testapplications.accelerometer;
...
public class MainActivity extends AppCompatActivity  implements SensorEventListener, AdapterView.OnItemSelectedListener {
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (printData.isChecked() ) {
            ((TextView)findViewById(R.id.magX)).setText(String.format("%.5f",event.values[0]));
            ((TextView)findViewById(R.id.magY)).setText(String.format("%.5f", event.values[1]));
            ((TextView)findViewById(R.id.magZ)).setText(String.format("%.5f", event.values[2]));
        }
